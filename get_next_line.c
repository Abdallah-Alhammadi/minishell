...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/18 16:00:22 by abalhamm          #+#    #+#             */
/*   Updated: 2023/09/22 20:02:41 by naalzaab         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

char	*join_free_gnl(char *s1, char *s2)
{
	char	*tmp;
...
char	*read_line(int fd, char *fd_table)
{
	...
	while (!nl && char_read > 0)
	{
		...
		buffer[char_read] = 0;
		fd_table = join_free_gnl(fd_table, buffer);
		nl = ft_strchr(buffer, '\n');
	}
	free(buffer);
	return (fd_table);
