...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/18 17:10:00 by abalhamm          #+#    #+#             */
...
void	update_file(t_token **token, t_list *lst, t_ints *t_int, char *tmp)
{
	char	*expanded;
	int		delim;
...
int	heredoc(int num, t_token **token, t_list *lst, t_ints *t_int)
{
	...
	t_int->hd = start_heredoc(t_int, num);
	if (t_int->hd == -1)
		return (0);
	...
	while (1)
	{
		...
		(*token)->type = delim;
		update_file(token, lst, t_int, tmp);
	}
	return (close(t_int->hd), free(lim), 0);
}
